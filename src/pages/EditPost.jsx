...
export default function EditPost() {
  ...
  useEffect(() => {
    API.get(`/posts/${id}`).then(r => {
      ...
      setForm({
        ...
        category: p.category,
        tags: (p.tags || []).join(', '),
        coverColor: p.coverColor || '#667eea',
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await API.put(`/posts/${id}`, { ...form, tags: form.tags.split(',').map(t => t.trim()).filter(Boolean) });
      navigate('/dashboard');
    } catch (err) {
  ...
  return (
    <div style={{ maxWidth: '760px', margin: '0 auto', padding: '2rem 1rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem' }}>
        <h2 style={{ fontFamily: 'Lora, serif', fontSize: '1.8rem', color: '#1a1a2e' }}>✏️ Edit Post</h2>
      ...
      <div style={{ background: '#fff', borderRadius: '20px', padding: '2rem', border: '1px solid #eee', boxShadow: '0 4px 20px rgba(0,0,0,0.06)', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        ...
        <input placeholder="Short excerpt..." value={form.excerpt} onChange={e => set('excerpt', e.target.value)}
